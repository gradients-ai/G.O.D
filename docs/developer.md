...
### Production image prediction L2

Build `ops/docker/validator-diffusion.dockerfile` as
`gradientsio/image-evaluator:latent-l2-v1`. The image and validator must be deployed
together: the old pixel-result contract is no longer accepted. The image is built
locally by the build command; pushing and deploying are separate release actions.
The evaluator covers Z-Image, FLUX, Qwen-Image, Krea-2 Raw and Ideogram-4; SDXL is
excluded. Production computes a scalar mean of 50% captioned and 50% empty-caption
flow-prediction MSE using shared 16 × 16 cases per retained test image per mode.
Only held-out test images count; pass training data for duplicate screening.

Basilica passes `TEST_SPLIT_URL` and `TRAIN_SPLIT_URL`; local Docker passes the
mounted test dataset and training URL. Both execute `/app/start.sh`, which invokes
the direct evaluator without starting a ComfyUI server or running separate model
preparation. Model assets use the Hugging Face cache. No base-model reward score,
pixel loss, reconstruction loss or generalization-gap penalty enters production.

Results carry metric version, component means, image identities and a fingerprint
of the evaluation cases/configuration. The fingerprint is persisted in the existing
`task_nodes.eval_set_fingerprint` column for image tasks. Comparisons containing
new and legacy scores or different new fingerprints fail closed; re-evaluate all
submissions in such a task before finalizing. All-legacy historical rankings remain
readable. Task ranking and tournament reward weights are otherwise unchanged.

See `ops/docker/README.md` for build/run commands and environment controls, and
`core/models/image_models.py` for the versioned result contract.
