...
from validator.tournament.models import PreparedRepo
from validator.tournament.models import RepoRef
from validator.tournament.repo_diff_report import _clone_repo
from validator.tournament.repo_diff_report import _collect_files
...
def _god_repo_root() -> Path:
    """Repo root of the validator's running source (validator/infrastructure/repo_dedup.py -> root)."""
...
def _build_runtime_context(has_source: bool) -> str:
    ...
    return (
        ...
        "./_god_source/trainer/runtime.py (run_trainer_container_text / run_trainer_container_image / "
        ...
        "'reward_functions' under ./_god_source; e.g. validator/tasks/rewards/templates.py, "
        "validator/tasks/rewards/affine.py, core/models/reward_models.py, core/models/dataset_models.py, "
        "and the --reward-functions handling in trainer/runtime.py). Reward functions receive `completions`, `extra_data` and "
...
def _parse_verdict(text: str) -> tuple[DupRelationship, float, str]:
    """Extract a verdict from the model's reply.

    The judge is asked for a bare JSON object, but can wrap it in code fences or
    emit small JSON-like malformations. Try strict JSON first, then regex fields.
    """
...
async def run_pairwise_dedup(repos: list[RepoRef], boss_hotkey: str | None = None) -> DedupResult:
    ...
    try:
        ...
        sem = asyncio.Semaphore(t_cst.TOURN_DEDUP_CONCURRENCY)
...
def render_report(
    ...
    Published reports omit DISTINCT pair reasons by default because those reasons can
    describe non-flagged miners' training innovations. Full verdicts stay in the DB.
    """
    lines = [
