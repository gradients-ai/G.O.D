...
async def process_non_stream_fiber_get(endpoint: str, config: Config, node: Node) -> dict[str, Any] | None:
    ...
    try:
        ...
        logger.error(
            f"Failed to communicate with node {node.node_id} ({node.hotkey}) ({server_address}{endpoint}): {repr(e)}"
        )
        return None

    if response.status_code != 200:
        logger.warning(
            f"Node {node.node_id} ({node.hotkey}) ({server_address}{endpoint}) returned HTTP {response.status_code}: {response.text[:200]}"
        )
        return None
...
async def call_content_service_fast(
    ...
    from validator.tasks.details import retry_http_fast
