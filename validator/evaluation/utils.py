...
def build_sglang_health_check(startup_minutes: int = 30) -> HealthCheckConfig:
    ...
    """
    initial_delay = 480  # 8 minutes - skip most of download phase
    period = 120  # 2 minutes - reasonable probe interval
    timeout = 120  # 2 minutes - allows for slow /health responses under load
...
def deploy_sglang_basilica(
    ...
    # Calculate timeout: startup window + 60s buffer
    startup_max = (
...
def build_env_health_check(startup_minutes: int = 5) -> HealthCheckConfig:
