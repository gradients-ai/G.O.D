...
def _get_training_hours_from_num_rows(num_rows: int, model_id: str | None = None, task_type: TaskType | None = None) -> float:
    ...
    base = data_cst.TRAINING_HOURS_MIN + t * (data_cst.TRAINING_HOURS_MAX_BASE - data_cst.TRAINING_HOURS_MIN)
    hours = max(data_cst.TRAINING_HOURS_MIN, round(base * 2) / 2)
    ...
    if model_id is not None:
        ...
        if num_params is not None and num_params < data_cst.FULL_HOURS_MODEL_PARAMS:
            ...
            hours = max(data_cst.TRAINING_HOURS_MIN, round(hours * scale * 2) / 2)

    type_mult = data_cst.TASK_TYPE_HOURS_MULTIPLIER.get(task_type, 1.0) if task_type is not None else 1.0
    hours = max(data_cst.TRAINING_HOURS_MIN, round(hours * type_mult * 2) / 2)
...
def apply_baseline_ctx_scale(hours: float, baseline_stats) -> float:
    ...
    ctx_scale = max(data_cst.CTX_SCALE_MIN, min(data_cst.CTX_SCALE_MAX, (packed_len / data_cst.CTX_REF_SEQ_LEN) ** 2))
