...
async def generate_style_synthetic(config: Config, num_prompts: int) -> tuple[list[ImageTextPair], str]:
    ...
    model_id = random.choice(synth_cst.FAL_STYLE_MODELS)
...
async def generate_person_synthetic(num_prompts: int) -> tuple[list[ImageTextPair], str]:
    ...
    image_prompt_pairs = await generate_fal_images_for_prompts(synth_cst.FAL_AVATAR_MODEL, prompts, face_image_url)
    ...
    return image_text_pairs, synth_cst.PERSON_SYNTH_DS_PREFIX


def _random_image_competition_hours() -> float:
    """Pick competition length in 15-minute (0.25h) steps between min and max."""
    min_q = int(round(synth_cst.MIN_IMAGE_COMPETITION_HOURS * 4))
    max_q = int(round(synth_cst.MAX_IMAGE_COMPETITION_HOURS * 4))
...
async def create_synthetic_image_task(config: Config, models: AsyncGenerator[ImageModelInfo, None]) -> RawTask:
    ...
    Path(TEMP_PATH_FOR_IMAGES).mkdir(parents=True, exist_ok=True)
    if random.random() < synth_cst.PERCENTAGE_OF_IMAGE_SYNTHS_SHOULD_BE_STYLE:
        ...
        for attempt in range(synth_cst.PERSON_GEN_RETRIES):
            ...
            if len(image_text_pairs) >= 10:
                break
            elif attempt < synth_cst.PERSON_GEN_RETRIES - 1:
