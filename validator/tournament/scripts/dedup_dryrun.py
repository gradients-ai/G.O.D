...
Usage (on the validator, with .vali.env sourced for DATABASE_URL + ANTHROPIC_API_KEY):
    python -m validator.tournament.scripts.dedup_dryrun <tournament_id>            # T0+T1 only (no Claude)
    python -m validator.tournament.scripts.dedup_dryrun <tournament_id> --full     # + T2 Claude pairwise
"""
...
async def main() -> None:
    ...
    if args.full:
        result = await run_pairwise_dedup(repos, boss_hotkey=cst.EMISSION_BURN_HOTKEY)
    else:
