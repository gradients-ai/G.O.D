...
- `task_creator.py`: tournament task creation. Text round 1 is always a single group with `SMALL_TOURNAMENT_GROUP_TASKS` instruct matches. The boss-round large instruct slot (35B–71B) skips augmentation so model-prep does not upload a full 70B copy. Prep-failure replacement of that slot redraws from the same size band.
