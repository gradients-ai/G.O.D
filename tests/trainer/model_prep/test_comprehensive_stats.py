...
class TestTrainingDynamics:
    ...
    def test_throughput_probe_shape(self, baseline_stats):
        assert baseline_stats.throughput is not None
        assert baseline_stats.throughput.tokens_per_sec > 0
        assert baseline_stats.throughput.seq_len > 0
        assert baseline_stats.throughput.micro_batch_size > 0
...
class TestJsonRoundtrip:
    def test_serialise_deserialise(self, baseline_stats):
        ...
        assert restored.throughput.tokens_per_sec == baseline_stats.throughput.tokens_per_sec
