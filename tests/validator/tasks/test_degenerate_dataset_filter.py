...
def stub_near_dup_rates(monkeypatch):
    """Stub the near-duplicate history lookup so the pre-filter can be exercised on fixed rates."""
...
async def test_near_dup_no_history_is_allowed(stub_near_dup_rates):
    """An unmeasured dataset must get one prep so its rate lands in baseline_stats."""
    stub_near_dup_rates([])
...
async def test_near_dup_uses_max_not_latest_or_mean(stub_near_dup_rates):
    """One prior failing measurement is enough: the mean here is well under the gate."""
...
async def test_near_dup_boundary_mirrors_orchestrator_gte(stub_near_dup_rates):
    """The orchestrator rejects at >=, so exactly-at-threshold must be skipped too."""
