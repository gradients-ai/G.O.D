...
class TestLoadTokenCounter:
    def test_uses_hf_when_loadable(self, monkeypatch):
        monkeypatch.setattr(tc, "AutoTokenizer", SimpleNamespace(from_pretrained=lambda repo: StubTokenizer()))
    ...
    def test_falls_back_on_load_error(self, monkeypatch):
        ...
        monkeypatch.setattr(tc, "AutoTokenizer", SimpleNamespace(from_pretrained=boom))
    ...
    def test_non_repo_id_skips_load_entirely(self, monkeypatch):
        ...
        monkeypatch.setattr(tc, "AutoTokenizer", SimpleNamespace(from_pretrained=must_not_call))
