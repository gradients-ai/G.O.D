...
async def scenario_1_perfect_performance():
    ...
    with patch("validator.scoring.weights.get_latest_completed_tournament", side_effect=[text_tournament, image_tournament]):
        with patch("validator.scoring.weights.get_latest_completed_tournament", side_effect=[prev_text, prev_image]):
...
async def scenario_5_only_text_tournament():
    ...
    with patch(
        "validator.scoring.weights.get_latest_completed_tournament", side_effect=[text_tournament, None]
    ):  # None for IMAGE
        with patch("validator.scoring.weights.get_latest_completed_tournament", return_value=prev_text):
            burn_data = await get_tournament_burn_details(mock_db)
