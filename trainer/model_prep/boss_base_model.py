...
from trainer.model_prep.lora_utils import check_for_lora
from trainer.model_prep.lora_utils import download_lora_with_retry
from trainer.model_prep.lora_utils import download_model_with_retry
from trainer.model_prep.lora_utils import merge_base_and_lora
...
def prepare_boss_base_model(aug_config: AugmentationConfig, repo_id: str, hf_token: str):
    ...
    is_lora = check_for_lora(source_repo, hf_token=hf_token, local_files_only=False)
    print(f"Preparing boss base model: repo={source_repo}, is_lora={is_lora}", flush=True)

    model_source = source_repo
    if is_lora:
        base_path = download_model_with_retry(aug_config.source_base_model_id, hf_token=hf_token)
        lora_dir = f"/tmp/model_prep_lora/{aug_config.source_task_id or 'source'}"
        download_lora_with_retry(source_repo, lora_dir, hf_token=hf_token)
        model_source = merge_base_and_lora(base_path, lora_dir, "/tmp/model_prep_merged")
